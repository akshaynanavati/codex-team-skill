...
- Use the optimize inspection command from [commands.md](commands.md) before choosing files to edit.
...
- Read only the active member's tasks and messages when context staleness is unclear.
- Never read another member's inbox and never read CEO inbox in this mode.

## Step 1: Understand The Member Contract

- Extract the member's recurring responsibilities, constraints, and decision rights from `ROLE.md`.
...
- Keep an eye out for information that should be promoted into its own topical file because it recurs.
